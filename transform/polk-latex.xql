(:~
    ...
    ODD: /db/apps/polk-papers/resources/odd/polk.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/polk/latex";
...
let $node := 
            ...
            return
                    ...
                    case element(pb) return
                        (: A possible place to override the default generation of the '[PAGE ]'. All values are still defaults. :)
                    ...
                    case element(ref) return
                        if (not(@target)) then
                            ...
                            if (not(text())) then
                                ...
                                latex:link($config, ., ("tei-ref3"), ., concat('?id=', @target))
